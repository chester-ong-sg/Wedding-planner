...
- **Fallback:** a skipped note, a missing key, a timeout (52s), a truncated reply, malformed JSON or a reply that doesn't match the plan shape all fall back to the built-in plan, so onboarding never dead-ends. Output is validated by `lib/wedding-plan/validate-plan.ts` before it is saved.
